...
def sheets_for_case(case: str, outputs: Path, out_dir: Path, zoom_mm: float) -> list[dict]:
    ...
    for rec in res.get("sinuses", []):
        ...
        if not (0 <= z < nz and 0 <= y < ny and 0 <= x < nx):
            continue
        plane = BEST_PLANE.get(rec["name"], 1)
        ...
        png = out_dir / f"{case}__{name}.png"
...
def main() -> int:
    ...
    for c in cases:
        print(f"{c}:")
        all_rows.extend(sheets_for_case(c, args.outputs_dir, args.out_dir, args.zoom_mm))
    if not all_rows:
