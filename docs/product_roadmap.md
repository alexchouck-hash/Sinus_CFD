...
| 3. Real Navier–Stokes CFD | **In progress** | Full pipeline runs end-to-end in Docker OpenFOAM on P001: CT → passage → geometry export → **watertight prism-layer mesh** (259k cells, checkMesh OK, skewness 3.2) → `simpleFoam` laminar → nasal resistance R=ρΔP/Q (`scripts/compute_nasal_resistance.py`). First genuine (non-stale) number: **R=0.052 Pa·s/mL**, below the published 0.10–0.35 range — three candidate explanations not yet distinguished (flow-rate nonlinearity vs. rhinomanometry's ~150 Pa test condition, cropped FOV missing the vestibule, or continued under-resolution). `docs/stage3_cfd_methodology.md` |
