...
| 3. Real Navier–Stokes CFD | **In progress (resistance validated)** | Full pipeline runs end-to-end in Docker OpenFOAM on P001: CT → passage → geometry export → **watertight prism-layer mesh** (259k cells, checkMesh OK, skewness 3.2) → `simpleFoam` laminar → nasal resistance R=ρΔP/Q (`scripts/compute_nasal_resistance.py`). Flow sweep (18/36/60/90 L/min) shows physiological Rohrer nonlinearity: **at ΔP≈150 Pa (rhinomanometry's own condition) R=0.156 Pa·s/mL, inside the published 0.10–0.35 range** — the resting R=0.052 is simply a lower operating point, not an error (`scripts/flow_sweep_report.py`). **Mucosal cooling** (passive-T solve): inspired air 20°C→**34.4°C** at the nasopharynx (85% conditioning, 4.9 W heat loss), matching published ~32–34°C with no tuning; resistance unchanged confirms passive coupling (`scripts/compute_mucosal_cooling.py`). **Mesh-independent**: at 75k/259k/817k cells ΔP changes 3.5%→**0.3%** (11x cells, <5% tol), so R=0.052 is validated not resolution-dependent (`scripts/mesh_independence_report.py`). Remaining: spatial wall-heat-flux map (tooling built, run pending). `docs/stage3_cfd_methodology.md` |
| **4. Virtual surgery loop** | **Done (geometry); CFD pre/post ready** | Parameterized turbinate reduction / septoplasty edit the airway, compared pre/post. P065 septoplasty `--side auto` picks the obstructed side, left MCA 9.1→19.3 mm², **L/R ratio 0.46→0.99** (asymmetric→balanced); P001 correctly flags that operating on a symmetric nose *creates* asymmetry. Edited label feeds the Stage-3 CFD for pre/post resistance. `docs/stage4_virtual_surgery.md`, `scripts/virtual_surgery.py` |
